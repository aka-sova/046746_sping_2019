...
for layer=1:layers_num    
    ...
    if (layer==1)
        ...
        log_pyramid{layer} = im2double(image)-im2double(imfilter(image,gauss_filter));
    ...
    elseif (layer == layers_num)
        ...
        sigma = 2^layer;
        gauss_filter = fspecial('gaussian',sigma*5,sigma); 
        log_pyramid{layer} = im2double(imfilter(image,gauss_filter));
        ...
        log_pyramid{layer} = im2double(imfilter(image,gauss_filter_1))-im2double(imfilter(image,gauss_filter_2));
    end
end
